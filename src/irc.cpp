...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("SustainCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SustainCoinTEST2\r");
            Send(hSocket, "WHO #SustainCoinTEST2\r");
        } else {
            // randomly join #SustainCoin00-#SustainCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SustainCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SustainCoin%02d\r", channel_number).c_str());
        }
