SustainCoin-qt: Qt4 GUI for SustainCoin
===============================
...
Alternatively, install Qt Creator and open the `SustainCoin-qt.pro` file.

An executable named `SustainCoin-qt` will be built.
...
A warning for people using the *static binary* version of SustainCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SustainCoin is linked against libdb5.3.
...
installed causes SustainCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
